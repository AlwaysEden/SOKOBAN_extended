...
static gboolean draw_callback(GtkWidget *widget, cairo_t *cr, gpointer data) {
	...
	if(img_x < img_y){
		...
		for(int j = 0; j < 3; j++){
			int alpha = j*cell_size;
			if(board[ ((img_y+alpha)/2)/10][converted_x] != 0) continue;
    ...
    int img_x = 1 * cell_size;
    int img_y = 10 * cell_size;
    draw_image(cr, img_x, img_y, cell_size, cell_size, player_path);
    board[(img_x/2)/10][(img_y/2)/10] = 1;
